...
export default function Page() {
  ...
  return (
    <div className="wrap">
      ...
      <p className="tagline">
        Got a suspicious email? Paste it below and get a plain-language verdict —
        no cybersecurity background needed. Anyone can use this, for free.
      </p>
      ...
      <div className="field">
        <label className="field-label" htmlFor="raw">
          Step 1 — Paste the email
        </label>
        <textarea
          id="raw"
          value={raw}
          onChange={(e) => setRaw(e.target.value)}
          placeholder={`Just copy the email text and paste it here — that's enough to run most checks.\n\nWant deeper checks (sender authentication)? Paste the full original source instead:\nGmail: open the email \u2192 \u22ee \u2192 "Show original" \u2192 copy all\nOutlook: open the email \u2192 \u22ee \u2192 "View message source"`}
        />
      </div>

      <details className="advanced">
        <summary>Step 2 — Add the real sender's domain (optional, improves accuracy)</summary>
        <div className="field" style={{ marginTop: 14 }}>
          <input
            ...
            placeholder="e.g. yourbank.com, yourcompany.com"
          />
          <p className="field-hint">
            If you know who the email is supposed to be from, add their real domain here — this
            catches lookalikes like "yourbank-secure.com" or "y0urbank.com". Skip this if you're
            not sure; the other checks still run.
          </p>
        </div>
      </details>
      ...
      <footer>
        PhishGuard is free to use. No account, no tracking, nothing ever leaves your browser.
      </footer>
    </div>
  );
}
